...
segment_gm_if_does_not_exist(){
  ...
  if [[ -e $FILESEGMANUAL ]]; then
    ...
    sct_qc -i ${file}$EXT -s ${FILESEG}$EXT -p sct_deepseg_gm -qc ${PATH_QC} -qc-subject ${SUBJECT_UNDERSCORE_SESSION}
     ...
     echo "Found segmentation!"
    rsync -avzh $FILESSEGPATH ${FILESEG}$EXT
    sct_qc -i ${file}$EXT -s ${FILESEG}$EXT -p sct_deepseg_gm -qc ${PATH_QC} -qc-subject $ 
  else
    echo "Not found. Proceeding with automatic segmentation."
    # Segment spinal cord
...
compute_snr_cnr(){
   ...
   #calculate SNR/CNR over entire volume
   gmM=$(fslstats ${file}${EXT} -k ${file_gmseg}${EXT} -M)
   gmS=$(fslstats ${file}${EXT} -k ${file_gmseg}${EXT} -S)
   wmM=$(fslstats ${file}${EXT} -k ${file_wmseg}${EXT} -M)
   wmS=$(fslstats ${file}${EXT} -k ${file_wmseg}${EXT} -S)

   CNR=$(echo "scale=5; ($wmM - $gmM)/ $wmS" | bc)
   SNR_wm=$(echo "scale=5; $wmM/$wmS" | bc)
   SNR_gm=$(echo "scale=5; $gmM/$gmS" | bc)
   echo ${file}","$wmM","$gmM","$wmS","$CNR >> ${PATH_RESULTS}/CNR/${SUBJECT_SLASH_SESSION}/avg_results.csv
   echo ${file}","$wmM","$wmS","$SNR_wm >> ${PATH_RESULTS}/SNR/${SUBJECT_SLASH_SESSION}/wm_avg_results.csv
   echo ${file}","$gmM","$gmS","$SNR_gm >> ${PATH_RESULTS}/SNR/${SUBJECT_SLASH_SESSION}/gm_avg_results.csv
...
for acq in "${ACQ[@]}";do
  for rec in "${REC[@]}";do
    ...
    if [ -e "${file}${EXT}" ]; then
      ...
      #always use the navigated gm segmentation (manually corrected) to calculate SNR/CNR for both standard and navigated images
